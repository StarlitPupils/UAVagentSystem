# E:/UAVagent/det_compare_sahi_v3.py
"""SAHI v3 检测对比"""
import sys, os
...
sahi_v3 = SAHIInferenceV3(vs.ensemble, finetuned_model_idx=0,
                          slice_size=640, overlap=0.3, slice_conf=0.12,
                          merge_iou=0.35, max_dets=200)
...
for fi in range(MAX_FRAMES):
    ...
    # SAHI v3
    dets_sahi = sahi_v3.detect(frame)
    tp_s = match_dets(dets_sahi, gt)
    sahi_tp += tp_s; sahi_fp += len(dets_sahi)-tp_s; sahi_fn += n_gt-tp_s
    
    if fi % 5 == 0:
        print(f"F{fi:3d}: GT={n_gt:2d} Base={len(dets_base):3d}(TP={tp_b}) SAHIv3={len(dets_sahi):3d}(TP={tp_s})")

print(f"\n{'='*55}")
print(f"SAHI v3 对比 (30帧, GT={base_tp+base_fn})")
print(f"{'':>8} {'基础融合':>10} {'SAHI v3':>10} {'变化':>10}")
...
print(f"\n  每帧平均检测数: 基础={base_tp+base_fp:.0f}  SAHIv3={sahi_tp+sahi_fp:.0f}")